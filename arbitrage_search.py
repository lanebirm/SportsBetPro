...
EMAIL_NOTIFY_THRESHHOLD = 1 # min value of the bet oppertunities max_value to prompt emailing notification
...
def main():
	...
	if PULL_API_DATA:
		...
		active_sports = ['americanfootball_ncaaf','americanfootball_nfl','basketball_euroleague','basketball_nba', 'basketball_ncaab', 'cricket_test_match','icehockey_nhl','mma_mixed_martial_arts', 'cricket_big_bash', 'rugbyleague_nrl', 'rugbyunion_six_nations', 'rugbyunion_super_rugby', 'tennis_atp_aus_open_singles', 'tennis_wta_aus_open_singles']
	...
	for odds in response_data['odds_data']:
		...
		data_processor[count].sort_max_h2h_odds(sites_best, sites_safety, exclude_sites)
		data_processor[count].generate_odds_total_value()
		data_processor[count].check_for_h2h_odds_total_value(TOTAL_VALUE_THRESHOLD)
	...
	for i in range(len(data_processor)):
		if len(data_processor[i].bet_opportunities['odds data']) > 0:
			results_print_statement = results_print_statement + 'Arbitrage opportunity for ' + data_processor[i].sports_name + ' for the following: \n '
			for j,event in enumerate(data_processor[i].bet_opportunities['odds data']):
				results_print_statement = results_print_statement + "Match Details: " + str(event) + "  ||  Total odds 'value': " + str(data_processor[i].bet_opportunities['odds total value'][j]) + '  ||  Event Time: ' + str(data_processor[i].bet_opportunities['start time'][j]) + "  ||  Time until: " + str(data_processor[i].bet_opportunities['time till start'][j]) + "\n"
		else:
			results_print_statement = results_print_statement + 'No Arbitrage opportunities for ' + str(data_processor[i].sports_name)
	...
	if EMAIL_NOTIFY:
		for i in range(len(data_processor)):
			if data_processor[i].bet_opportunities['max value'] > EMAIL_NOTIFY_THRESHHOLD:
				...
				SimplyNotify.email('Free Money to be made', results_print_statement, 'lanebirmbetnotify@gmail.com')
				break
